...
const uploadFields = upload.fields([{ name: 'font1', maxCount: 1 }, { name: 'font2', maxCount: 1 }])

// GET /api/tipografias - listar todas las tipografías
router.get('/', async (req, res) => {
    try {
        const result = await pool.query('SELECT id_tipografia, nombre, tamanio_titulo, tamanio_subtitulo, tamanio_parrafo, font1, font2, created_at FROM tipografia ORDER BY created_at DESC');
...
router.put('/:id', uploadFields, async (req, res) => {
    try {
        ...
        if (req.is('multipart/form-data') || req.files) {
            ...
            tamanio_titulo = req.body && req.body.tamanio_titulo ? parseInt(req.body.tamanio_titulo, 10) : null
            tamanio_subtitulo = req.body && req.body.tamanio_subtitulo ? parseInt(req.body.tamanio_subtitulo, 10) : null
            tamanio_parrafo = req.body && req.body.tamanio_parrafo ? parseInt(req.body.tamanio_parrafo, 10) : null
